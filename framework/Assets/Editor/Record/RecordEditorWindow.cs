...
public class RecordEditorWindow : EditorWindow
{
    ...
    void LoadRecord(string recordName)
    {
        if (m_currentRecordName != recordName)
        {
            ...
            if (m_currentRecordName != "None")
                m_currentRecord = RecordManager.Instance.GetData(recordName);
        }
    }
    bool isRecordFold;
    string recordName = "";
        ...
        if (isRecordFold)
        {
            ...
            if (!m_recordNameList.Contains(recordName) && recordName != "")
            {
                ...
                if (GUILayout.Button("新增", GUILayout.Width(position.width - 60)))
                {
                    ...
                    RecordManager.Instance.SaveData(recordName, dict);
                    FindAllRecordName();
    ...
    void DeleteRecordGUI()
    {
        if (m_currentRecordName != "None")
        {
            if (GUILayout.Button("删除数据"))
            {
                ...
                {
                    File.Delete(Application.dataPath + "/Resources/" + RecordManager.m_directoryName + "/" + m_currentRecordName + ".json");
    ...
    void RecordEditorGUI()
    {
            ...
            && m_currentRecordName != "None")
        {
            ...
            {
                RecordManager.Instance.SaveData(m_currentRecordName, m_currentRecord);
                AssetDatabase.Refresh();
            }
    ...
    void FindAllRecordName()
    {
        ...
        FindRecordName(Application.persistentDataPath + "/" + RecordManager.m_directoryName);
    ...
    public void FindRecordName(string path)
    {
        ...
        {
            if (item.EndsWith("." + RecordManager.m_expandName))
            {
